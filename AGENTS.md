...
> **Status: charter, not architecture.** This directory is empty as of 2026-06-11. There is no
> codebase yet. This file captures *intent, constraints, and the candidate landscape* so future
> sessions start oriented. It will be expanded with real architecture sections **only after** an
> architecture is selected and built — do not back-fill invented structure before then.
...
These are distinct and live at different layers. Keep them separate in everything you write here.
...
When the doc later says "the architecture," it must say *which* of these two it means.
...
- **Three layers.** `raw/` (immutable curated sources the LLM reads but never edits) → `wiki/`
  ...
  log) → **schema** (a `AGENTS.md`/`AGENTS.md` that defines structure + the ingest/query/lint ops).
...
| [lucasastorian/llmwiki](https://github.com/lucasastorian/llmwiki) | Codex (via MCP) | — | Upload docs, connect Codex via MCP, it writes the wiki. Heavier (MCP) — tension with the "simplistic/local" constraint. |
| [toolboxmd/karpathy-wiki](https://github.com/toolboxmd/karpathy-wiki) | Codex | — | Codex skills for compounding KBs. |
...
- A selected architecture that demonstrably works across the named agents (not just Codex).
...
- This `AGENTS.md` — the schema. As the project takes shape, extend it with the ingest/query/lint
...
vs. the four constraints. Meaningless against an empty wiki — expand when content exists.
